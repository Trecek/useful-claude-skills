...
### Data Flow Insights
1. **Irreversible transformations**: Filtering, normalization, scaling modify `.X` in-place unless `copy=True`
2. **Cumulative state**: Each stage adds new slots without removing old ones (except `.X` overwrite)
3. **Critical bifurcation**: `.raw` freeze creates two parallel data tracks (full vs. HVG)
4. **Lazy evaluation**: UMAP/t-SNE depend on PCA but don't require specific ordering between them
5. **Graph-centric clustering**: All clustering algorithms depend on pre-computed neighborhood graph

### Common Pitfalls
1. **Missing .raw**: Calling `rank_genes_groups()` without setting `.raw` → error or wrong results
2. **Wrong order**: Calling `umap()` before `pca()` → falls back to using `.X` directly (inefficient)
3. **Scale before HVG**: Scaling on full gene set → wasted computation, `.raw` freezes scaled data
4. **Overwrite layers**: Not preserving original counts in `.layers['counts']` → can't re-normalize

### Best Practices
1. Always freeze `.raw` immediately after `log1p()`
2. Store original counts in `.layers['counts']` before any normalization
3. Use `copy=True` for exploratory branches to avoid mutating main object
4. Check `.isbacked` before in-place operations on large datasets
5. Use `.write()` to checkpoint after expensive operations (PCA, neighbors)
